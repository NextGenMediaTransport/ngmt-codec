...
- **Linux CI (GCC):** for **x86_64**, CMake passes **`-msse4.2`** to `src/vmxcodec.cpp` and `src/vmxcodec_x86.cpp` on GCC/Clang so SSE4.1 intrinsics (`smmintrin.h`) match the translation unit ISA (default generic `x86-64` is SSE2-only and caused “target specific option mismatch” on Ubuntu).
